...
# -------------------- IMAGE TAB --------------------
    if mode == "Image":
        ...
        if f:
            ...
            # image accident alert (once per image)
            ...
            if acc_max is not None and acc_max >= acc_alert:
                caption = (
                    ...
                    f"Conf: {acc_max:.2f}\n"
                    f"Run: {run_id}\n"
                    f"{presign_s3_uri(s3_uri)}"
                )
                alert_mgr.send_photo(str(out_path), caption)
    ...
    try:
            while True:
                    ...
                    if idx % every_n == 0:
                        ...
                        # Accident persistence → Telegram with best frame
                        ...
                        if (not sent_accident_alert) and len(window) == persist_k and sum(window) == persist_k:
                            ...
                            caption = (
                                ...
                                f"S3: {presign_s3_uri(s3_img_uri) if s3_img_uri else ''}"
                            )
                            if s3_uri_video:
                                caption += f"\nVideo: {presign_s3_uri(s3_uri_video)}"
                            alert_mgr.send_photo(frame_to_send, caption)

                            st.error(
                                f"🚨 Accident in video (≥{acc_alert:.2f}) with {persist_k} consecutive frames. "
                                "Best frame sent to Telegram."
                            )
